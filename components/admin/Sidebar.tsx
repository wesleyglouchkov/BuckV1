...
export default function AdminSidebar({ mobileOpen, setMobileOpen }: {
	...
	return (
		<TooltipProvider>
			...
			{/* Desktop sidebar */}
			<aside
				className={`hidden md:flex flex-col fixed left-0 top-16 h-[calc(100vh-4rem)] border-r border-border/20 bg-card overflow-hidden shrink-0 transition-[width] duration-300 ease-out z-20 shadow-sm ${collapsed ? "w-16" : "w-64"
					}`}
			>
					...
					<ul className="space-y-1">
						{adminMenu.map(({ icon: Icon, label, href }) => {
							...
							const linkContent = (
								<Link
									href={href}
									className={`flex items-center ${collapsed ? 'justify-center px-2' : 'gap-2 px-2.5'} py-2 transition-colors ${isActive
									...
									return (
										<li key={href}>
											<Link
												href={href}
												onClick={toggleMobile}
												className={`flex items-center gap-2 rounded-none px-3 py-2 transition-colors ${isActive
														? "bg-foreground text-background"
														: "text-muted-foreground hover:bg-accent hover:text-foreground"
													}`}
											>
